...
import { Mail, Phone, MapPin, Send, MessageCircle, CheckCircle, Sparkles, Linkedin } from 'lucide-react'
import { FadeInUp, FadeInLeft, FadeInRight, StaggerContainer, StaggerItem, FloatingElement, PulseGlow, SlideInFromBottom } from '@/components/EnhancedAnimations'
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          <div className="space-y-8">
            <div className="glass-strong p-8">
              <h3 className="text-3xl font-semibold mb-8 gradient-text">Let's Connect</h3>
              ...
              <div className="space-y-6">
                {contactInfo.map((info, index) => (
                  ...
                  >
                    <div className={`w-14 h-14 ${info.color} bg-white/10 rounded-2xl flex items-center justify-center mr-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
                      {info.icon}
                    </div>
                    <div>
                      <h4 className="font-semibold text-white mb-1 group-hover:text-blue-300 transition-colors duration-300">{info.title}</h4>
                      <p className="text-white/70 group-hover:text-white/90 transition-colors duration-300">{info.value}</p>
                    </div>
                  </a>
                ))}
              </div>
          ...
          <div className="glass-strong p-10 relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="group">
                <label htmlFor="name" className="block text-sm font-medium text-white/90 mb-3 group-focus-within:text-blue-400 transition-colors duration-300">
                  Name
                </label>
                <input
                  type="text"
              ...
              <div className="group">
                <label htmlFor="email" className="block text-sm font-medium text-white/90 mb-3 group-focus-within:text-blue-400 transition-colors duration-300">
                  Email
                </label>
                <input
                  type="email"
              ...
              <div className="group">
                <label htmlFor="message" className="block text-sm font-medium text-white/90 mb-3 group-focus-within:text-blue-400 transition-colors duration-300">
                  Message
                </label>
                <textarea
                  id="message"
